...
4. Run this workflow for <root>/CHANGELOG.md, and also for <root>/*/CHANGELOG.md if existing and relevant
5. Sync the same updated semantic version across all relevant files, e.g.:
   - <root>/pyproject.toml (`project.version`, 3rd line), or
   - <root>/apps/*: **both** root `package.json` and `apps/*/package.json` (`version`, 3rd line) — keep these aligned
